...
決定木は、貪欲法で選択された個々のテストを使用して変数空間を分割する分類器です。クラスを分離する際に、最も高い情報量を持つ分割を見つけることを目的としています。決定木は、多数の分割（木の深さ）を有効にすることで簡単に過学習状態になるため、多くの場合、正則化（例えば、枝刈りやモデルの複雑性を制限するハイパーパラメータの指定）が必要です。Pythonでは、sklearnのDecisionTreeClassifier実装を使用します。 \index{決定木} \index{Python}
...
モデル & 勾配ブースティンマシンで、ハイパーパラメーターは下記の通りとします：木の数　5000 \\
\end{longtable}

\begin{description}
\item[、木の最大の深さ　4、7、または10　学習速度　0.001、0.01 、0.1、または0.9。共変量は、性別、年齢、コンディション、薬剤、薬剤グループ、ビジット数を含みます。 データ分割: 75％トレーニング - 25％テスト、個人ごとにランダムに割り当てられます。 \textbar{}]
\label{tab:plpSummary} この研究の主なデザイン選択
\end{description}
