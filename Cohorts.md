...
例えば、コンセプトセットの表現は、図に示されるように2つのコンセプトを含むことができます（表 \@ref(tab:conceptSetExpression)）。ここでは、コンセプト[4329847](http://athena.ohdsi.org/search-terms/terms/4329847)（「心筋梗塞」）とそのすべての下位層に含まれるコンセプトを含みますが、コンセプト[314666](http://athena.ohdsi.org/search-terms/terms/314666)（「陳旧性心筋梗塞」）とそのすべての子孫を除外します。
