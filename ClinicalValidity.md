...
3.  xSpecとxSensコホートを使用して予測モデルを適合：第 \@ref(PatientLevelPrediction) で説明したように、幅広い患者の特徴量を予測因子として使用してモデルを適合し、その人物がxSpecコホート（結果が出ると考えられる人々）に属するのか、あるいはxSensコホート（結果が出ないと考える人々）の逆数に属するのかを予測することを目指します。
...
\caption{PheValuatorを使用したコホート定義の性能特性の決定 p(O) = 結果の確率; TP = 真陽性; FN = 偽陰性; TN = 真陰性; FP = 偽陽性}(\#fig:phevaluatorDiagram)
\end{figure}
