...
3.  xSpecとxSensコホートを使用して予測モデルを適合：第\@ref(PatientLevelPrediction)章 で説明したように、幅広い患者の特徴量を予測因子として使用してモデルを適合し、その人物がxSpecコホート（結果が出ると考えられる人々）に属するのか、あるいはxSensコホート（結果が出ないと考える人々）の逆数に属するのかを予測することを目指します。
