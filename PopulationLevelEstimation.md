...
\caption{自己対照コホートデザイン。ターゲットへの曝露中の結果の発生率を曝露前の時間中の発生率と比較します。}(\#fig:scc)
\end{figure}
...
\caption{症例対照デザイン。結果を持つ被験者（「症例」）は、結果を持たない被験者（「対照」）との曝露ステータスの面で比較されます。通常、症例と対照は年齢や性別などの様々な特性で一致させられます。}(\#fig:caseControl)
\end{figure}
...
\caption{症例交叉デザイン。結果の周りの時間を、結果の日付より前の事前に決められた間隔のコントロール日と比較します。}(\#fig:caseCrossover)
\end{figure}
...
\caption{ネガティブコントロール概念セット。}(\#fig:ncConceptSet)
\end{figure}
...
\caption{除外する概念を定義する概念セット。}(\#fig:covsToExclude)
\end{figure}
...
\caption{ネガティブコントロールおよび除外する概念セットを示す比較ウィンドウ。}(\#fig:comparisons2)
\end{figure}
...
\caption{脱落図。上部に示されているカウントは目標および比較対象コホートの定義を満たしているものです。下部に示されているカウントは、結果モデルに入るものです。この場合、Cox回帰です。}(\#fig:attrition)
\end{figure}
