...
\caption{ネガティブコントロール（真のハザード比=1）およびポジティブコントロール（真のハザード比> 1）に関する推定値。各点はコントロールを表します。点線の下にある推定値は、真の効果量を含まない信頼区間を持ちます。}(\#fig:controls)
\end{figure}
...
\caption{P値のキャリブレーション：破線以下の推定値は従来のp < 0.05。網掛け部分の推定値はキャリブレーションされたp < 0.05。網掛け部分の端の狭い帯は95\%信用区間。点はネガティブコントロール。ダイアモンド型は対象のアウトカムを示します。}(\#fig:pValueCal)
\end{figure}
...
\caption{Methods LibraryのMethodsに対する95\%信頼区間のカバー率。各ドットは分析選択の特定セットの性能を表します。点線は名目上の性能（95\%カバー率）を示します。SCCS = 自己対照症例シリーズ、GI = 消化管、IBD = 炎症性腸疾患。}(\#fig:methodEval)
\end{figure}
