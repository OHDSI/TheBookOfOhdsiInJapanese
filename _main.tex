...
\item
  主たる入院コンディション（Condition Primary Inpatient）： condition\_occurrenceテーブルで入院患者の主たる診断として観察されたコンディションごとのバイナリ共変量。
\item
  薬剤： 選択された薬剤コンセプトIDと時間間隔ごとに共変量を構築し、DRUG\_ERAテーブルのコホート開始日前の指定された時間間隔に、患者がコンセプトIDを記録している場合、共変量の値は 1 、そうでない場合は 0。
\item
  処置（プロシージャー）：選択されたプロシージャーコンセプトIDと時間間隔ごとに共変量を構築し、PROCEDURE\_ERAテーブルのコホート開始日前の指定された時間間隔に、患者がコンセプトIDを記録している場合、共変量の値は 1 、そうでない場合は 0。
\item
