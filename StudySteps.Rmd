...
研究集団の定義が記述されると、OHDSIツールATLASは関連するコホートを作成するための良い出発点となります。ATLASとコホート作成プロセスについては、章 \@ref(OhdsiAnalyticsTools)と章 \@ref(Cohorts)で詳しく説明します。簡単に説明すると、ATLASは詳細な包含基準を持つコホートを定義・生成するためのユーザーインターフェース（UI）を提供します。一旦ATLASでコホートが定義されると、ユーザーはプロトコルに組み込むために、その詳細な定義を人間が読める形式で直接エクスポートすることができます。何らかの理由でATLASインスタンスが観察健康データベースに接続されていない場合でも、ATLASを使用してコホート定義を作成し、SQLデータベースサーバー上で個別に実行される研究パッケージに組み込むために、基礎となるSQLコードを直接エクスポートすることができます。ATLASはコホート定義のSQLコードを作成する以上の利点があるため、可能であればATLASを直接使用することをお勧めします（下記参照）。最後に、コホート定義をATLAS UIで実装することができず、手作業でカスタムSQLコードを作成する必要がある場合も稀にあります。

ATLAS UIは多くの選択基準に基づいてコホートを定義することができます。コホートへの入室と退室の基準、およびベースラインの基準は、OMOP CDMのドメインに基づいて定義することができ、各ドメインには標準コードを指定する必要があります。さらに、これらのドメインに基づいた論理的なフィルターや、試験期間やベースラインの時間枠を定義するための時間ベースのフィルターもATLAS内で定義することができます。ATLASは各基準のコードを選択する際に特に役立ちます。ATLASには、コホートの定義に必要なコードのセットを構築するために使用できる語彙ブラウジング機能が組み込まれています。この機能は、OMOP 標準語彙のみに依存し、語彙階層内のすべての子孫を含めるオプションがあります（章 \@ref(StandardizedVocabularies)参照）。そのため、この機能では、ETL プロセスですべてのコードが標準コードに適切にマッピングされてい る必要があることに注意してください（章 \@ref(ExtractTransformLoad)を参照）。包含基準で使用する最適なコードセットが明確でない場合、コホート定義で探索的分析が正当化される場 合があります。あるいは、より正式な感度分析を行い、異なるコードセットを用いたコホートの様々な定義の可能性を考慮することも考えられます。

ATLASがデータベースに接続するように適切に設定されていると仮定すると、定義されたコホートを生成するSQLクエリをATLAS内で直接実行することができます。ATLASは自動的に各コホートに一意なIDを割り当てますが、これは将来使用するために、バックエンドのデータベースでコホートを直接参照するために使用することもできます。コホートはATLAS内で発生率研究を実行するために直接使用することもできますし、PLEまたはPLP研究パッケージのコードによってバックエンドデータベースで直接参照することもできます。あるコホートについて、ATLASはコホート内の個人の患者ID、インデックス日、コホート終了日のみを保存します。この情報は、特性評価、PLEまたはPLP試験のための患者のベースラインの共変量など、患者について必要とされる可能性のある他のすべての属性または共変量を導出するのに十分です。

コホートが作成されると、患者の属性や最も頻繁に観察される薬剤や病態の頻度などのサマリー特性が作成され、デフォルトでATLAS内で直接表示することができます。

実際には、ほとんどの研究で複数のコホートまたは複数のコホートセットを指定する必要があり、それらを様々な方法で比較することで、新たな臨床的洞察を得ることができます。PLEとPLPの場合、OHDSIツールはこれらの複数のコホートを定義するための構造化されたフレームワークを提供します。例えば、PLEの比較有効性試験では、通常少なくとも3つのコホート、ターゲットコホート、コンパレータ、アウトカムコホートを定義します (章 \@ref(PopulationLevelEstimation)を参照のこと)。さらに、完全なPLE比較有効性試験を実施するためには、陰性対照アウトカムと陽性対照アウトカムを持ついくつかのコホートも必要です。OHDSIツールセットには、これらの陰性対照コホートと陽性対照コホートの生成を高速化し、場合によっては自動化するための方法が用意されています（詳細については、章 \@ref(MethodValidity)で述べられています）。

最後に、研究のためのコホートを定義することは、OHDSIコミュニティで進行中の強固で検証された表現型のライブラリを定義する作業から恩恵を受けるかもしれません。これらの既存のコホート定義のいずれかがあなたの研究に適している場合、正確な定義はATLASインスタンスにjsonファイルをインポートすることで取得できます。

### 実施可能性と診断 {#Feasibility}

\index{study feasibility!single study} \index{study diagnostics}

いったんコホートが定義され、作成されれば、利用可能なデータソースにおける研究の実施可能性を検討する、より正式なプロセスを実施し、その結果を最終的なプロトコールにまとめることができます。試験の実施可能性の評価には、多くの探索的な、時には反復的な活動が含まれます。ここではいくつかの一般的な側面について説明します。

この段階での主な活動は、作成したコホートが望ましい臨床的特徴と一致していることを確認し、予期しない特徴にフラグを立てるために、コホート内の特徴の分布を徹底的に検討することです。上記のT2DMの例に戻りますが、この単純なT2DMコホートの特徴を、他のすべての診断の頻度をレビューすることによって、T1DM患者や他の予期しない問題を含む患者を捕捉することにフラグを立てることができるかもしれません。コホート定義の臨床的妥当性の品質チェックとして、このような新しいコホートを最初に特徴付けるステップを研究プロトコールに組み入れることは良い習慣です。実施に関しては、ATLASでコホートが作成された時にデフォルトで生成されるコホートの人口統計学と上位の薬剤と病態を調べるのが一番簡単な方法です。ATLASでコホートを直接作成するオプションが利用できない場合は、手動でSQLを実行するか、Rの特徴抽出パッケージを使用してコホートを特徴付けることができます。実際には、大規模なPLE研究やPLP研究では、これらのステップを特徴抽出ステップと一緒に研究パッケージに組み込むことができます。

PLEまたはPLPの実施可能性を評価するもう一つの一般的で重要なステップは、コホートサイズと対象コホートと比較対象コホートにおけるアウトカムの数の評価です。ATLASの罹患率機能は、これらのカウントを求めるために使用することができ、別のところで説明されているように、検出力計算に使用することができます。

PLE研究で強く推奨されるもう1つのオプションは、傾向スコア（PS）マッチングのステップと関連する診断を完了し、対象群と比較群の集団の間に十分な重複があることを確認することです。これらのステップについては \@ref(PopulationLevelEstimation)の章で詳しく説明しています。さらに、これらの最終的なマッチングされたコホートを用いて、統計的検出力を計算することができます。

場合によっては、OHDSIコミュニティでは、利用可能なサンプルサイズが与えられた場合の最小検出可能相対リスク（MDRR）を報告することによって、研究が実行された後にのみ統計的検出力を検討します。このアプローチは、多くのデータベースやサイトにわたって、高スループットで自動化された研究を実行する場合に有用です。このシナリオでは、任意のデータベースにおける研究の検出力は、事前にフィルタリングするよりも、すべての解析が実行された後に調査する方が良いかもしれません。
