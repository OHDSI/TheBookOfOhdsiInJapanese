...
SQL Serverなどの一部のDBMSプラットフォームは常に大文字と小文字を区別せずに文字列比較を行いますが、PostgreSQLなどの他のプラットフォームは常に大文字と小文字を区別します。したがって、常に大文字小文字の区別を前提に比較を行い、大文字小文字の区別が不明な場合は明示的に大文字小文字の区別をしないようにすることをお勧めします。例えば、次のように：
