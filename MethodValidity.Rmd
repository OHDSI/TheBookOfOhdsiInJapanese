...
同様に、95%信頼区間のカバー率は95%未満であることが一般的に観察されます：真の効果量は95%信頼区間の95%未満にあります。信頼区間較正[@schuemie_2018]のために、我々はポジティブコントロールも利用することで、p値較正のフレームワークを拡張します。一般的に、必ずしもそうではありませんが、較正された信頼区間は公称信頼区間よりも広くなります。これは、標準的な手順では考慮されていない問題（未測定の交絡、選択バイアス、測定誤差など）が、較正では考慮されていることを反映しています。

形式的には、ペア$i$に関連するバイアスである$beta_i$は、再びガウス分布に由来すると仮定しますが、今度は、真の効果量である$theta_i$と線形関係にある平均と標準偏差を使用します：
...
異なる集団、異なる医療システム、異なるデータ収集プロセスを表す複数の異なるデータベースを用いて研究を実施することも、方法の検証の1つです。先行研究では、異なるデータベースで同じ研究デザインを実施すると、効果量の推定値が大きく異なることが示されており[@madigan_2013]、母集団によって効果量が大きく異なるか、異なるデータベースで見られる異なるバイアスに適切に対処していないことが示唆されています。実際、信頼区間の経験的な較正を通じてデータベースの残留バイアスを考慮することで、研究間の異質性を大幅に減らすことができます[@schuemie_2018]。

データベース間の異質性を表現する1つの方法は$I^2$スコアで、研究間の全変動のうち偶然ではなく異質性に起因する割合を記述します。[higgins_2003]$I^2$の値を素朴に分類することは、すべての状況に対して適切ではありませんが、$I^2$の値が25％、50％、75％の場合、暫定的に低、中、高の形容詞を割り当てることができます。[schuemie_2018b]は、大規模な傾向スコア調整を用いた新規ユーザーコホートデザインを用いて多くのうつ病治療の効果を推定した研究で、$I^2$が25%未満であった推定値は58%しかありませんでした。経験的較正の結果、これは83%に増加しました。

```{block2, type='rmdimportant'}
データベース間の異質性が観察されると、推定値の妥当性が疑われます。残念ながら、逆は真ではありません。異質性が観察されないからといって、バイアスのない推定値が保証されるわけではありません。すべてのデータベースが同じようなバイアスを共有し、すべての推定値が一貫して間違っている可能性は低くありません。
```
...
研究を計画する際、しばしば不確実なデザインの選択があります。例えば、傾向スコアマッチングと層別化のどちらを用いるべきか？層別を用いる場合、層数はいくつにするか？適切なタイムアットリスクは？このような不確実性に直面した場合、1つの解決策は様々な選択肢を評価し、デザイン選択に対する結果の感度を観察することです。様々な選択肢の下でも推定値が変わらない場合、その研究は不確実性に対して頑健であると言えます。

この感度分析の定義は、感度分析を「様々な大きさの隠れたバイアスによって、研究の結論がどのように変化するかを評価する」と定義している\@rosenbaum のような他の人が使っている定義と混同してはいけません。
...
: (#tab:exampleMetrics) Method performance metrics derived from the negative and positive control estimates.
...
: (#tab:sensAnalysis) Uncalibrated and calibrated hazard ratios (95% confidence interval) for the two analysis variants.
